 ...
 * For a customer, active debt is the sum over CONFIRMED `credit` transactions
 * of (transaction.amount − sum of that transaction's CONFIRMED payments),
 * clamped at 0. Draft/cancelled transactions and non-confirmed payments are
 * excluded. Customers with no qualifying transactions are absent from the
 * returned Map; callers should treat a missing key as 0.
 */
...
export async function getActiveDebtByCustomer(businessId, customerId = null) {
  ...
  for (const tx of transactions) {
    const outstanding = netOutstanding(tx.amount, tx.payments);
    debts.set(tx.customerId, (debts.get(tx.customerId) ?? 0) + outstanding);
  }
  return debts;
}
